...
function change_status(agent_name, myid) {
    var circle = document.getElementById(myid).style.color;

    fetch('/change_agent_status_admin/', {
        method: 'POST',
        headers: {
            'Content-type': 'application/json',
            'X-CSRFToken': csrftoken,
        },
        body: JSON.stringify({ 'details': [agent_name] })
    })
    .then((resp) => resp.json())
    .then(response => {

        // console.log("AGENT STATUS RES: ", response);
...
function uuid_transfer(user_uuid, from_room, to_room, application) {

    console.log("DROP TRANSFER: ", user_uuid, from_room, to_room, application)
    ...
    fetch('/pbx/uuid_transfer/', {
            ...
            body: JSON.stringify({ 'details': [user_uuid, from_room, to_room, application] })
        })
        .then((resp) => resp.json())
        .then(response => {
...
function getRoomType(divId) {
    // Get the div element by its ID
    var div = document.getElementById(divId);

    // Check if the div has the class 'call_queue'
    if (div.classList.contains('call_queue')) {
        return 'callcenter';
    }

    // Check if the div has the class 'conference_room'
    if (div.classList.contains('conference_room')) {
        return 'conference';
    }

    // If neither class is present, return null or any default value
    return 'weird';
}
    ...
    $(".dropable-room").droppable({
        ...
        drop: function(event, ui) {
            ...
            if (type == "room_banner") {
                ...
                var call_application = getRoomType(to_room);
                ...
                console.log("ORIGINAL:", [user_uuid, from_room, to_room, call_application]);

                // socket.send(JSON.stringify({'details':[caller_name_id, from_room, to_room]}));

                uuid_transfer(user_uuid, from_room, to_room, call_application);
    ...
    .then(function(Responsedata) {
        ...
        for (let i = 0; i < crs.length; i++) {
            ...
            if (user_data[1] != 1943) {
                ...
                room_html = `
            <div class="scrollbar conference_room dropable-room col-first col style-3" id="${user_data[1]}">
    ...
    .then(function(Responsedata) {
        ...
        for (let i = 0; i < crs.length; i++) {
            ...
            if (user_data[4] != 1943) {
                ...
                room_html = `
            <div class="scrollbar call_queue dropable-room col-first col style-3" id="${user_data[4]}">
                <div class="room-heading ho" id="${user_data[0]}">
...
function forward_room_uuid(my_data) {

    banner_uid = my_data.split('.')[1]

    remove_banner(banner_uid);

    fetch('/pbx/uuid_transfer_fwd/', {
            method: 'POST',
            headers: {
                'Content-type': 'application/json',
                'X-CSRFToken': csrftoken,
...
function kill_call(banner_uuid) {

    b_uuid = banner_uuid.replace("hangup-", "")

    console.log("BANNER UUID: ", b_uuid);
    ...
    fetch('/pbx/uuid_kill/', {
        ...
        body: JSON.stringify({ 'details': b_uuid })
    })
    .then((resp) => resp.json())
    .then(response => {

        console.log("CALL KILL RES: ", response);

        if (response.includes("Success")) {
            remove_banner(banner_uuid);
        } else {
            console.log("ERROR REMOVING CALL: ", response)
        }
    ...
    for (const dict_key in conferences) {
        ...
        if (room != '1943' && conferences[dict_key]['users']) {
            ...
            callers.forEach(users_obj => {
                ...
                var agent_elem = removePrefixIfExists(users_obj['cid_num'], country_codes);

                if (users_obj['uuid'] && !(users_obj['uuid'] in active_users) && users_obj['call_state'] == "active") {

                    const container = document.getElementById(room);

                    const myDiv = (container && users_obj['uuid']) ? container.querySelector(`#users_obj-${users_obj['uuid']}`) : null;

                    if (users_obj['uuid'] in ringing) {
                        change_ball(agent_elem, "green");

                        delete ringing[users_obj['uuid']];
                    }
                            ...
                            <div ondblclick = "set_note(this.id)" style = "background-color:${my_background}; color:${my_font};" class="container draggable_banner noselect room-banner users_obj-${users_obj['cid_name']}" id="users_obj-${users_obj['uuid']}">
                                ...
                                <div class="user_uuid menu-wrap" id = "${users_obj['uuid']}">
                                    ...
                                    <div class="menu">
                                        <div>
                                            <ul>
                                                <li><a class="link myurl" id="${users_obj['cid_name']}-url" href="${users_obj['url']}" target="_blank">Url</a></li>
                                                ...
                                                <li><a class="link" id = 'hangup-${users_obj['uuid']}' onclick = "kill_call(this.id)" > Hangup </a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                    ...
                    if (users_obj['call_state'] == "active") {
                        console.log("USER EXIST: ", users_obj['uuid']);
                    
                        old_room = active_users[users_obj['uuid']][0];

                        console.log("CURRENT ROOM: ", users_obj['dest'], "OLD ROOM: ", old_room);

                        if (old_room != users_obj['dest']) {
                            remove_banner(users_obj['uuid']);
                        } 
                    } else if (users_obj['call_state'] == "early") {

                        let agent_element = document.getElementById(`status-${agent_elem}`);

                        if (agent_element !== null) {
                            agent_element.style.color = "yellow";

                            ringing[users_obj['uuid']] = `status-${agent_elem}`;
                        }
...
function change_ball(ball_id, ball_color) {
    let agent_element = document.getElementById(`status-${ball_id}`);

    if (agent_element !== null) {
        agent_element.style.color = ball_color;
    }
}

function removePrefixIfExists(inputString, stringList) {
    for (let prefix of stringList) {
                ...
                if ('add_note' in data) {
                    ...
                    for (let key in notes) {
                        if (notes.hasOwnProperty(key)) {
                            banner_ref = key.replace("users_obj-", "");
                            banner_note = notes[key];

                            get_banner = document.getElementById(banner_ref);

                            if (get_banner) {
                                get_banner.innerHTML = banner_note;
                            }
                        }
                    }
                }
...
function remove_banner(banneruuid) {
    ...
    // socket.send(JSON.stringify({ 'message': "dump_banner", 'uid':banneruuid}));
