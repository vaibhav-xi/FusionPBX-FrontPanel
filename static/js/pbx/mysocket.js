...
function set_note(bannerid) {
    double_clicked = bannerid.replace('-click', '');
    ...
    .then(response => {
        ...
        if (total_active_calls != "No Queues") {
            ...
            for (const dict_key in total_active_calls) {
                ...
                callers = values(d[dict_keys]['users']);
...
socket.onopen = function() {
  ...
  socket.onmessage = function(event) {
    ...
    if (keys(mydict).length === 0) {
      mydict = data;
  
    } else if (JSON.stringify(mydict) != JSON.stringify(data)) {
        ...
        for (const dict_key in data) {
            ...
            callers.forEach(element => {
                ...
                if (myDiv) {
                    ...
                    raw_html = `
                        <div ondblclick = 'set_note(this.id)' style = "background-color:${data[dict_key]['background']}; color:${data[dict_key]['font']};" class="container draggable_banner noselect room-banner" id="element-${element[1]}">
                            <div class="text" id="${element[1]}">${element[0]}</div>
                            <div class="menu-wrap">
                                <input type="checkbox" class="toggler">
                                <div class="dots">
